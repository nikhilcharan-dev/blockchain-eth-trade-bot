...
export default function MainChart() {
    ...
    useEffect(() => {
            ...
            try {
                ...
                const bids = Array.isArray(data.bids)
                    ? data.bids
                        ...
                        .slice(0, 12)
                    : [];
                let asks = Array.isArray(data.asks)
                    ? data.asks
                        .map(toLevel)
                        .filter(Boolean)
                ...
                let safeBids = bids;
                if (safeBids.length === 0 && asks.length === 0) {
                    const t = wazirxPrices[selectedSymbol];
                    if (t && Number.isFinite(t.bidPriceInr) && Number.isFinite(t.askPriceInr)) {
                        safeBids = [{ price: t.bidPriceInr, qty: Math.max((t.volume || 1) / 1000, 0.0001) }];
                        asks = [{ price: t.askPriceInr, qty: Math.max((t.volume || 1) / 1000, 0.0001) }];
                    }
                }

                setOrderBook({ bids: safeBids, asks });
        ...
        return () => clearInterval(iv);
    }, [symbolPair, selectedSymbol, wazirxPrices]);
                ...
                ws.onmessage = (event) => {
                    try {
                        ...
                        if (Array.isArray(message?.data)) {
                            const entry = message.data.find((row) => row?.s === symbolPair);
                            const candidate = parseFloat(entry?.c ?? entry?.lastPrice ?? entry?.last);
    ...
    const refreshChartNow = () => {
        fetchHistory();
    };

    const refreshOrderBookNow = async () => {
        try {
            const resp = await fetch(`${WAZIRX_DEPTH_URL}?symbol=${symbolPair}&limit=30`, { cache: "no-store" });
            if (!resp.ok) return;
            const data = await resp.json();
            ...
            let bids = Array.isArray(data.bids)
                ? data.bids.map(toLevel).filter(Boolean).sort((a, b) => b.price - a.price).slice(0, 12)
                : [];
            let asks = Array.isArray(data.asks)
                ? data.asks.map(toLevel).filter(Boolean).sort((a, b) => a.price - b.price).slice(0, 12)
                : [];

            if (bids.length === 0 && asks.length === 0) {
                const t = wazirxPrices[selectedSymbol];
                if (t && Number.isFinite(t.bidPriceInr) && Number.isFinite(t.askPriceInr)) {
                    bids = [{ price: t.bidPriceInr, qty: Math.max((t.volume || 1) / 1000, 0.0001) }];
                    asks = [{ price: t.askPriceInr, qty: Math.max((t.volume || 1) / 1000, 0.0001) }];
                }
            }
            ...
            console.error("Manual order book refresh error:", err);
        }
    };
        ...
        <div className="eth-chart">
            ...
            <div className="eth-chart-container">
                <div className="main-chart-title-row">
                    ...
                    <button className="mini-refresh-btn" onClick={refreshChartNow}>Refresh Chart</button>
                    <button className="mini-refresh-btn" onClick={refreshOrderBookNow}>Refresh Book</button>
                </div>
